 ...
 */
@Slf4j
public class HttpProxyInboundHandlerV3 extends SimpleChannelInboundHandler<HttpObject> {
    ...
    public HttpProxyInboundHandlerV3(Map<String, PacModel> blackList, ConfigProperties configProperties, EventLoopGroup eventExecutors) {
    ...
    public void channelRead(ChannelHandlerContext channelHandlerContext, Object msg) throws Exception {
        ...
        Object msgToNext = proxyConnect(host, port, method, req);
        TrojanWrapperRequest trojanWrapperRequest = this.getTrojanWrapperRequest(msgToNext, IpUtil.parseAddress(host).byteValue(), host, port, configProperties.getTrojanPassword());

        channelHandlerContext.fireChannelRead(trojanWrapperRequest);
    }
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, HttpObject msg) throws Exception {
        ...
        if (pacMode.isBlock()) {
            ...
            ctx.pipeline().remove(HttpProxyInboundHandlerV3.class);
            ctx.pipeline().fireChannelRead(msg);
            return;
        }

        Object msgToNext = null;
        switch (ProxyModelEnum.get(configProperties.getProxyMode())) {
            ...
            case PAC:
                if (pacMode.isProxy()) {
                    msgToNext = proxyConnect(host, port, method, req);
                } else {
                    directConnect(host, port, method, req);
                }
                break;
    ...
    private Object proxyConnect(String host, int port, HttpMethod method, HttpRequest req) {
        if (port == -1) {
            port = 80;
        }
                ...
                public void operationComplete(Future<Channel> channelFuture) throws Exception {
                    ...
                    ctx.writeAndFlush(resp).addListener(new ChannelFutureListener() {
                        ...
                        public void operationComplete(ChannelFuture future) throws Exception {
                            ...
                            p.remove(HttpProxyInboundHandlerV3.class);
                        }
                    });;
                    ChannelPipeline p = ctx.pipeline();
    ...
    private TrojanWrapperRequest getTrojanWrapperRequest(Object msg, int atyp, String dstAddr, int dstPort, String password) {
